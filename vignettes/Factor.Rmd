---
title: "Factor Copula → TrialSimulator: End-to-End Example (Factor Gaussian Copula)"
...
This vignette demonstrates how to wrap the `simdatacopulas` factor copula (Factor Gaussian copula) joint generator into the `generator(n)` interface required by `TrialSimulator`, and how to:

1. Define high-dimensional dependence via a factor structure (`loadings`, `uniq_var`) (low-rank + diagonal)
...
## 2. Define Factor Structure and Marginals (dist), Build `simdesign`
...
Placebo and treatment share the same factor-copula dependence structure (same `loadings` and `uniq_var`), but use different marginal parameters to represent a "treatment effect".
...
## 3. Wrap into TrialSimulator `generator(n)`
...
## 4. TrialSimulator: Define endpoints/arms/trial/milestones and run
...
## 5. Read Locked Data and Run Basic Checks
